...
export const useLocalStorage = (clave, valorInicial = []) => {
  ...
  const guardarValor = (valorNuevo) => {
    // valorNuevo => va a contenedr un producto = {}
  ...
  const actualizarValores = (carrito) => {
    try {
      ...
      console.error(
        `Error al actualizar el carrito ${clave} del localStorage: ${error}`
      );
    }
  };
  ...
  const limpiarStorage = () => {
    try {
      ...
      console.error(
        `Error al limpiar el Storage ${clave} del localStorage: ${error}`
      );
    }
  };
